# Présentation professionnelle

## 1. Qui je suis

Ingénieur logiciel et architecte applicatif avec plus de 18 ans d’expérience dans des environnements critiques (énergie, télécom, industrie), j’ai construit ma carrière autour de trois axes majeurs :

- l’architecture logicielle et la conception de plateformes,
- la coordination transverse entre équipes techniques, DSI, sécurité et exploitation,
- l’industrialisation, la qualité logicielle et la fiabilité des systèmes.

J’ai évolué dans des contextes exigeants où la rigueur, la documentation, la sécurité et la stabilité opérationnelle sont essentielles.  
Mon rôle naturel : **architecte logiciel / DevSecOps transverse**, capable de structurer, sécuriser et faire évoluer des plateformes complexes.

---

## 2. Pourquoi ce projet

Après plusieurs années en architecture et coordination, j’ai souhaité **actualiser et consolider** mon socle technique autour des pratiques modernes du marché :

- Java 21 / Spring Boot 3,
- CI/CD GitHub Actions,
- Docker et images sécurisées,
- Kubernetes (kind puis EKS),
- sécurité applicative et DevSecOps,
- documentation professionnelle,
- pilotage d’IA dans un contexte d’ingénierie.

L’objectif n’était pas de changer de métier, mais de **mettre à jour mes compétences**, de démontrer ma capacité à mener un projet complet de bout en bout, et de formaliser une démarche professionnelle cohérente avec mon parcours.

Ce projet est donc une **consolidation**, pas une reconversion.

---

## 3. Ambitions du positionnement

Mon ambition est claire :  
**me positionner sur des rôles d’architecture logicielle, DevSecOps senior ou coordination transverse**, en apportant :

- une vision d’ensemble,
- une capacité à structurer et sécuriser,
- une maîtrise des pratiques modernes,
- une approche rigoureuse et documentée,
- une posture senior orientée qualité, stabilité et gouvernance.

Je souhaite évoluer dans des environnements où l’architecture, la sécurité, la CI/CD et la coordination sont au cœur des enjeux.

---

## 4. Démarche Secure by Design transverse

La démarche mise en œuvre repose sur l’intégration de la sécurité **dans les phases que je maîtrise réellement** :

**Architecture → Sécurité applicative → CI/CD → Sécurité d’exécution → Sécurité infrastructure**

Elle ne prétend pas couvrir la stratégie ou la GRC organisationnelle à ce stade, mais se concentre sur l’application concrète des bonnes pratiques de sécurité dans un projet d’ingénierie logicielle.  
Les futures releases prévoient d’étendre cette démarche en articulant les exigences sécurité applicatives avec les exigences issues de la GRC (NIS2, analyses de risques) et de la gouvernance/stratégie.

### 4.1 Architecture logicielle
- Conception d’une architecture moderne (Spring Boot 3, Docker, Kubernetes),
- Définition des patterns de sécurité applicative,
- Structuration des flux, zones de confiance et responsabilités.

### 4.2 Sécurité applicative (processus structuré)
La sécurité applicative est intégrée via un processus en trois niveaux :

1. **Fiches sécurité**  
   - Identification des exigences sécurité par composant,  
   - Définition des contrôles applicatifs (authN, authZ, validation, journalisation, secrets, etc.).

2. **Spécifications techniques**  
   - Déclinaison des exigences sécurité en mécanismes concrets,  
   - Patterns d’implémentation, contraintes d’architecture, contrôles à intégrer dans le code.

3. **Spécifications fonctionnelles**  
   - Référence aux exigences sécurité comme contraintes transverses,  
   - Pas d’implémentation sécurité dans les specs fonctionnelles (centrées sur le métier).

### 4.3 CI/CD & Supply Chain Security
- Pipeline GitHub Actions sécurisé,
- Analyse statique, SBOM, Trivy, VEX,
- Durcissement des images et gestion des dépendances.

### 4.4 Sécurité d’exécution
- Déploiement Kubernetes local (kind) puis cloud (EKS),
- NetworkPolicy, RBAC, secrets, observabilité.

### 4.5 Sécurité infrastructure
- Durcissement des environnements,
- Gestion des identités cloud,
- Séparation des rôles et privilèges.

---

## 5. Collaboration humain–IA (contrôle croisé)

La démarche intègre un modèle innovant de collaboration humain–IA :

- une IA de génération (code, architectures, documents),
- une IA de contrôle (sécurité, cohérence, conformité, qualité),
- un arbitrage humain systématique pour garantir la maîtrise et la robustesse des livrables.

Ce principe de **contrôle croisé IA → IA**, supervisé par l’humain, reproduit un modèle de double validation utilisé dans les environnements critiques et renforce la qualité de la démarche Secure by Design.

---

## 6. Processus professionnel et itératif

Le projet est structuré en **releases successives**, chacune documentée par :

- une spécification d’architecture versionnée,
- une spécification technique,
- une fiche sécurité,
- une release note,
- un schéma d’architecture (logique, applicative, déploiement).

Ce processus reproduit les pratiques d’une DSI moderne et démontre une capacité à **industrialiser la sécurité** dans les projets.

---

## 7. Ce que ce projet démontre

Ce projet démontre :

- ma capacité à concevoir une architecture moderne,
- ma maîtrise des pratiques DevSecOps,
- ma compréhension des enjeux sécurité,
- ma capacité à industrialiser un pipeline complet,
- ma compétence à déployer sur Kubernetes local et cloud,
- ma rigueur documentaire,
- ma capacité à piloter un projet complet,
- ma maturité dans la collaboration humain–IA.

Il constitue une preuve concrète de ma capacité à occuper un rôle d’**architecte logiciel / DevSecOps senior**.

---

## 8. Publication contrôlée

Le projet **n’est pas publié intégralement**.

Les artefacts internes (fiches sécurité, spécifications techniques, schémas complets, release notes détaillées) sont protégés et présentés uniquement en entretien.

Le dépôt public présente :

- la démarche,
- les objectifs,
- les principes Secure by Design,
- un schéma d’architecture non sensible,
- la structure générale du projet.

---

## 9. Ce que je recherche aujourd’hui

Je me positionne sur des rôles tels que :

- **Architecte logiciel**,  
- **Architecte DevSecOps**,  
- **Consultant SSI transverse**,  
- **Référent technique**,  
- **Coordinateur transverse DSI**,  
- **Ingénieur plateforme / CI/CD / Cloud**,  
- **Architecte cloud / Kubernetes**.

Avec un intérêt particulier pour les environnements :

- à forte exigence technique,
- nécessitant de la coordination transverse,
- où la sécurité et la qualité sont centrales,
- où l’architecture doit guider les équipes.

---

## 10. Conclusion

Mon parcours, enrichi par ce projet, me permet aujourd’hui d’apporter :

- une vision d’architecte,
- une expertise DevSecOps moderne,
- une rigueur issue des environnements critiques,
- une capacité à documenter, structurer et sécuriser,
- une posture senior orientée qualité et stabilité,
- une maîtrise de la collaboration humain–IA.

Je suis prêt à contribuer à des projets où l’architecture, la sécurité, la CI/CD et la coordination sont des enjeux majeurs.